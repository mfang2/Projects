...
function drawInitDataRow(row, length) { //drawing the board is hard
...
function drawRealDataRow(arr, row, length) { //use empty spaces if no player data
...
function drawHeadRow(size) {//spacing for top row
    var res = '  ';
    var sym = '';
...
function drawDivider(length) { //horizontal divider of shape '---+'
    var symbol = '---+';
    var res = '   ';
    for (var i=0; i<length; ++i) {
      if (i== length - 1) {
...
function create2DArray(size) { //Because JS does not natively have 2D arrays (??)
    var matrix = new Array(size);
    for (var i=0; i<size; ++i) {
      matrix[i] = new Array(size);
    }
  
    for (let i=0; i<size; ++i) {//each entry in first array contains an array
...
function getPlayerSymbol(playerNumber) {//as per instruction 2, available player symbols are X through Z
    const symb = 'XOABCDEFGHIJKLMNPQRSTUVWXYZ'
    let symbol = symb[playerNumber]; //get the index for the particular player
    return symbol;
}  
  
function checkHorizontal(arr, row, col, symbol) { //check win conditions horizontally
    var size = data.size; //larger size means larger check range
    let result = false;
    let seq = data.seq; //what the winning sequence should be
...
function checkVertical(arr, row, col, symbol) { //check vertically
    var size = data.size;
    let result = false;
    let seq = data.seq;
...
function checkDiagonal(arr, startRow, startCol, symbol, direction) { //much harder, testing / direction is harder than \ direction
...
function checkNorthWest(arr, row, col, symbol) {//check the furthest top-left corner
    let result = false;
...
function checkNorthEast(arr, row, col, symbol) {//check from top right box
    let result = false;
    let startRow, startCol;
    let size = data.size;
    console.log('row = ' + row);
...
function checkWin(arr, row, col, symbol) { //if any of the directions (horizontal, vertical, / or \) have win, return win
    let result1 = false;
    let result2 = false;
    let result3 = false;
    let result4 = false;
...
function checkActive(symbol) { //for testing and debugging at this poin.
...
function playGame(arr, currPlayer, playerInt, size) {
    let result = false; 
    let preSymbol = "";//which player won?
    while (currPlayer < playerInt) { //loop through each player's turn
      ...
      let currSymbol = getPlayerSymbol(currPlayer);
      ...
      */
      ...
      let input = readlineSync.question("Give me coordinates separated by a space or Q to quit "); //coordinates are given as 1 1, or 2 1, etc.
      if (input == 'Q' || input == 'q') {
  ...
  function Data(player, size, seq, curr, arr) {//object data structure to store and load game data to.
  ...
  function beginNewGame() { //to begin new game, need to know number of players, board size, and win number/sequence
    let player = readlineSync.question('How many players ? : ');
    let playerInt = parseInt(player);
    ...
    let size = readlineSync.question('What is the board size ? : ');
    ...
    let seq = readlineSync.question('What is the winning sequence ? : ');
    ...
    let arr = create2DArray(size); //if above data is okay, build the game board
    data.arr = arr;
    playGame(arr, 0, playerInt, sizeInt);  
  }
  
  function checkFileExistsSync(filepath){ //synchronous check if file exists
    let flag = true;
    try{
...
function resumeOldGame() {
    let found = false; //boolean if file can be found
    let fileName = readlineSync.question('Please provide the save game file name : '); 
